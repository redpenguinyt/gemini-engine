...
//! ```
//! Ok, let's go over this and see what's going on. We start by creating a [`View`] and [`Pixel`]. the [`View`] takes two numbers for the width and height, as well as a [`ColChar`](view::ColChar). The [`Pixel`] takes a [`Vec2D`] and a [`ColChar`](view::ColChar).
//!
...
//!
//! At its heart, [`Vec2D`] is just a pair of `isize` integers for defining things such as position, size and movement. We used it here to define the [`Pixel`]'s starting position, before the game loop.
//!
